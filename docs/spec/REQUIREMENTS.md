...
- **FairPlay is excluded from Ferry v1 and is not on the roadmap** unless Apple changes their licensing policy.
- Ferry can receive unencrypted AirPlay streams and AirPlay streams encrypted with the open AES-128-CTR session key mechanism. FairPlay-protected premium content (e.g., Apple TV+) will not play.
