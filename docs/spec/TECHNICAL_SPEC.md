...
Ferry registers two mDNS services:
...
│  ┌────────────────────────────▼───────────────────────────────────┐  │
│  │               FerryService  (ForegroundService)             │  │
...
|---|---|---|
| `FerryService` | `service/FerryService.kt` | ForegroundService: owns lifecycle of all protocol receivers; exposes `StateFlow` for UI |
...
| `HomeFragment` | `ui/HomeFragment.kt` | Protocol state cards; service control buttons; binds to FerryService |
...
|---|---|---|---|
| `googletv` | `com.ferry.receiver.googletv` | 29 | Google TV / Android TV, Leanback UI, may use newer APIs |
| `firetv` | `com.ferry.receiver.firetv` | 25 | Amazon Fire TV, must avoid Google-only APIs |
...
Ferry does **not** need to implement Widevine or PlayReady key exchange logic. The Cast SDK handles license acquisition, key delivery, and secure decryption. On devices without Widevine L1 (hardware-secured), content providers may restrict playback to Widevine L3 (software).
...
| Workaround | None within scope — users must use unprotected content for AirPlay to Ferry |
