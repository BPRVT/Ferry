...
| AC-0.5 | All 4 documents are committed to the project branch | `git log --oneline` | Commit containing all 4 files visible in log |
...
**Goal:** macOS discovers Ferry in the AirPlay picker within 3 seconds of app launch.

| # | Criterion | How to Verify | Pass Condition |
|---|---|---|---|
| AC-2.1 | Ferry appears in macOS AirPlay menu within 3 seconds | Start timer at app launch, watch macOS menu bar → AirPlay icon | Device name appears ≤ 3s after app launch |
...
| AC-2.3 | Ferry disappears from macOS picker within 10 seconds of app close | Close app, watch picker | Device removed from picker ≤ 10s after close |
...
| # | Criterion | How to Verify | Pass Condition |
|---|---|---|---|
| AC-3.1 | macOS completes RTSP OPTIONS → SETUP → ANNOUNCE → RECORD sequence | Ferry logcat shows all 4 RTSP methods received | All 4 methods logged, all responded with `200 OK` |
...
| AC-6.3 | RAM usage stays ≤ 150 MB throughout the 30-minute test | `adb shell dumpsys meminfo com.ferry.receiver.*` every 5 minutes | Peak ≤ 150 MB at every measurement |
| AC-6.4 | CPU usage stays ≤ 30% average throughout the 30-minute test | `adb shell top -p $(pidof com.ferry.receiver.*)` | 5-minute rolling average ≤ 30% |
| AC-6.5 | Automatic reconnect after network disruption (Wi-Fi toggle) | Disable and re-enable Wi-Fi on sender Mac | Ferry reappears in macOS picker within 5s of reconnect |
