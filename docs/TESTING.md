...
./gradlew test --tests "com.ferry.receiver.airplay.RtspHandlerTest"
```
...
./gradlew assembleGoogletvDebug -Pferry.castAppId=<APP_ID>
...
Console and associate the Android TV package `com.ferry.receiver.googletv`. See
...
FERRY_PACKAGE=com.ferry.receiver.firetv tools/collect-device-logs.sh
```
...
2. Launch Ferry from the TV app list
...
**Goal:** macOS discovers Ferry in the AirPlay menu within 3 seconds.

1. Ensure Mac and TV are on the same Wi-Fi network
2. Launch Ferry on the TV
...
6. Close Ferry (press Back on TV)
...
1. Connect macOS to Ferry as in Scenario 3
...
5. Check RAM: `adb shell dumpsys meminfo com.ferry.receiver.*` should show < 150MB
...
3. **Expected:** Ferry reappears in the macOS AirPlay menu within 5 seconds of network restoration
...
adb shell dumpsys meminfo com.ferry.receiver.googletv | grep "TOTAL"

# CPU usage (5-second average) — replace PID with actual process ID
adb shell top -n 5 -p $(adb shell pidof com.ferry.receiver.googletv) | tail -5
```
