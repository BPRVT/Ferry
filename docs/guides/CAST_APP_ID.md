...
Ferry cannot invent a Google Cast App ID locally. The ID is assigned by
...
with the Cast App ID in the console. Ferry's Google TV package is:

```text
com.ferry.receiver.googletv
```
...
```bash
FERRY_CAST_APP_ID=<APP_ID> ./gradlew assembleGoogletvDebug
```

If no ID is supplied, the Google TV build still succeeds, but Ferry marks
