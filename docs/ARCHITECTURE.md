...
This document explains how Ferry works — from the moment a macOS user clicks "AirPlay" to the moment video appears on the TV.
...
When Ferry starts, it uses a technology called **mDNS** (Multicast DNS) — the same thing as Apple's "Bonjour" — to broadcast a message on your local network. This message says:
...
|---|---|---|
| `FerryApp` | `FerryApp.kt` | App-level init (logging setup) |
| `MainActivity` | `MainActivity.kt` | Hosts the UI, manages app lifecycle |
| `FerryService` | `service/FerryService.kt` | ForegroundService — keeps receiver alive |
...
```
  ...
  ◄── mDNS response ────────────────────── ("Ferry" at 192.168.1.x:7000)

  AirPlay device appears in macOS menu ✓

  ─────── HANDSHAKE ──────────────────────────────────────────────────────

  [User clicks "Ferry" in AirPlay menu]
