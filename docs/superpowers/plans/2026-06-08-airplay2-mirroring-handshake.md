...
**Goal:** Make Ferry actually receive a macOS screen-mirroring session by implementing the AirPlay 2 handshake it currently lacks: `GET /info` → `pair-setup`/`pair-verify` → `fp-setup` (FairPlay) → encrypted `SETUP` → the encrypted H.264 mirroring stream, decoded to the existing `VideoDecoder`.

**Architecture:** Ferry already has the *back half* of a receiver (RTSP request reader, H.264 `VideoDecoder` → `Surface`, mDNS advertising). It is missing the *front half* — the HTTP-over-RTSP handshake macOS requires. We add that as a set of focused handler modules under `com.ferry.receiver.airplay.handshake`, route `GET`/`POST` by URI path in `RtspHandler`, add a binary-safe response writer, layer ChaCha20-Poly1305 encryption onto the control socket after `pair-verify`, port the reverse-engineered FairPlay key exchange, and stream the decrypted mirroring video into the existing decoder.
...
**New package `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/`:**
...
**Modified files:**
- `app/src/main/kotlin/com/ferry/receiver/airplay/RtspMessages.kt` — add binary body + content-type to `RtspResponse`.
- `app/src/main/kotlin/com/ferry/receiver/airplay/RtspHandler.kt` — binary `sendResponse`, `GET`/`POST` path routing, encryption hooks, mirror-setup wiring.
- `app/src/main/kotlin/com/ferry/receiver/airplay/AirPlayReceiver.kt` — wire mirror stream → `VideoDecoder`; provide the surface for mirroring (not only RECORD).
- `app/src/main/kotlin/com/ferry/receiver/airplay/MdnsService.kt` — add the `pk` (Ed25519 public key) and confirm `features`/`flags` advertise mirroring+pairing.
...
**Files:**
- Modify: `app/src/main/kotlin/com/ferry/receiver/airplay/RtspMessages.kt`
...
**Files:**
- Modify: `app/src/main/kotlin/com/ferry/receiver/airplay/RtspHandler.kt` (the `sendResponse` function, ~line 320)
...
**Files:**
- Modify: `app/src/main/kotlin/com/ferry/receiver/airplay/RtspHandler.kt` (the `routeRequest` `when`, ~line 148)
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/PlistCodec.kt`
- Test: `app/src/test/kotlin/com/ferry/receiver/airplay/handshake/PlistCodecTest.kt`
...
- [ ] **Step 1 (RED):** Write a test that a `Map<String,Any?>` encoded to a binary plist and decoded back is equal, and that the encoded bytes start with the bplist magic `"bplist00"`.
  ...
  @Test fun roundTripsMap() {
      val m = mapOf("name" to "Ferry", "n" to 7L, "b" to true)
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/InfoResponder.kt`
...
- [ ] **On-device checkpoint:** macOS still sends `POST /pair-setup` (and may now retry with a *larger* pair-setup body, indicating it accepted `/info`). Confirm no plist parse errors logged on the Mac side isn't observable, so rely on: Ferry logs `GET /info` then `POST /pair-setup` repeatedly without macOS giving up immediately.
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/PairingKeys.kt`
...
- [ ] **Step 3 (GREEN):** Generate an Ed25519 keypair with BouncyCastle (`Ed25519PrivateKeyParameters`), persist the 32-byte seed in SharedPreferences (`ferry_prefs`, key `pairing_ed25519_seed`), expose `ed25519Public: ByteArray`, `sign(data): ByteArray`.
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/Tlv8.kt`
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/PairSetupHandler.kt`, `PairVerifyHandler.kt`
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/ControlCipher.kt`
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/FairPlay.kt`
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/MirrorSetupHandler.kt`
...
**Files:**
- Create: `app/src/main/kotlin/com/ferry/receiver/airplay/handshake/MirrorStreamServer.kt`
