...
#
# Credentials are stored in ~/.config/ferry/release.env (outside the repo).

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
CREDS_FILE="${HOME}/.config/ferry/release.env"
KEYSTORE_FILE="${HOME}/.config/ferry/ferry-release.jks"
...
cmd_setup() {
    bold "=== Ferry release keystore setup ==="
    echo ""
    ...
    read -rp "Key alias (e.g. ferry): " KEY_ALIAS
...
cmd_release() {
    ...
    GOOGLETV_APK="Ferry-${VERSION}-googletv.apk"
    FIRETV_APK="Ferry-${VERSION}-firetv.apk"
