 ...
 *
 * WHY: For macOS/iOS to show Ferry in the AirPlay menu, the device must announce
 * itself using mDNS (Multicast DNS, the same protocol as Apple's Bonjour).
 * Without this advertisement, no sender would know Ferry exists.
 *
...
class MdnsService(
     ...
     * on the network is already registered as "Ferry", Android will register us as
     * "Ferry (2)" instead. The [onActualNameRegistered] callback delivers the name
     * that was actually registered (which may differ from the requested name).
     *
     * The caller can use this to update the UI (e.g., show "Registered as: Ferry (2)")
     ...
     *
     * Uses the Settings override if the user set one; otherwise advertises "Ferry".
     *
     * The default is deliberately the product name rather than the TV's system device name:
     * this string appears in the iPhone Control Center and macOS AirPlay menu next to real
     * Apple devices, so it needs to read as a device. It carries no suffix and no version.
     *
     * To advertise the TV's own system name instead, set Settings → Device Name to it
     * ([NetworkUtils.getDeviceName] still resolves it).
     */
    private fun resolveDisplayName(override: String?): String {
        val trimmed = override?.trim() ?: ""
        return if (trimmed.isNotEmpty()) trimmed else NetworkUtils.DEFAULT_DEVICE_NAME
    }

    /**
     * Registers the `_airplay._tcp` mDNS service.
     *
     * TXT records tell senders what features Ferry supports.
