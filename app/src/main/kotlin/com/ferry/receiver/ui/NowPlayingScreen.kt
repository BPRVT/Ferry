...
class NowPlayingScreen @JvmOverloads constructor(
    ...
    init {
        ...
        artwork = ImageView(context).apply {
            ...
            // Square, outlined surface so the placeholder glyph (and any non-square
            // art) sits on a frame. No corner radius — nothing in this UI is rounded.
            background = GradientDrawable().apply {
                setColor(color(R.color.background_surface))
                setStroke(dp(1), color(R.color.border_idle))
            }
            clipToOutline = true
        }
    ...
    // Built in code, so the theme's mono textViewStyle does not apply here — the
    // typeface is set explicitly to keep this screen consistent with the rest.
