...
/**
 * HomeFragment — The main screen of Ferry.
 *
 ...
 *
 * HOW: Binds to [FerryService] to receive real-time state updates.
...
class HomeFragment : Fragment() {
    ...
    // Service binding — gives direct access to FerryService StateFlows
    ...
    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, binder: IBinder?) {
            ...
            isBound = true
            Logger.d("HomeFragment: bound to FerryService")
            observeServiceState()
        }
        ...
        override fun onServiceDisconnected(name: ComponentName?) {
            service = null
            isBound = false
            Logger.d("HomeFragment: unbound from FerryService")
        }
    }
    ...
    override fun onStart() {
        ...
        val intent = Intent(requireContext(), FerryService::class.java)
     ...
     * Calls [ServiceController] which sends Intent actions to [FerryService].
     */
    ...
    /**
     * Starts collecting state updates from [FerryService].
    ...
    private fun updateServiceStateBadge(state: ServiceState) {
        ...
        dotServiceState.backgroundTintList = ColorStateList.valueOf(requireContext().getColor(colorRes))
    }
    ...
    private fun updateProtocolCard(card: View, state: ProtocolState) {
        ...
        dot.backgroundTintList = statusTint(colorRes)
    }

    /**
     * Tint for a status block inside a focusable cell.
     *
     * Two states, not one flat colour, for two reasons. When the cell is focused it
     * fills with the accent, and a green "running" block on the accent would vanish
     * — so the focused state flips the block to near-black. And using a tint list
     * rather than `background.setTint()` avoids mutating the shared drawable
     * ConstantState, which previously let one cell's colour leak into the other two.
     */
    private fun statusTint(colorRes: Int): ColorStateList {
        val states = arrayOf(
            intArrayOf(android.R.attr.state_focused),
            intArrayOf()
        )
        val colors = intArrayOf(
            requireContext().getColor(R.color.text_on_accent),
