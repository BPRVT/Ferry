...
object NetworkUtils {
     ...
     * 3. Fallback: "Ferry" (if neither source is available)
     *
    ...
    // Constants
    /**
     * The name advertised over mDNS when the user has not set one in Settings, and the
     * fallback when the system device name is unreadable. Appears in the iPhone Control
     * Center / macOS AirPlay menu, so: no suffix, no version string.
     */
    const val DEFAULT_DEVICE_NAME = "Ferry"
    private const val FALLBACK_MAC_ADDRESS = "aa:bb:cc:dd:ee:ff"
    private const val PREFS_NAME = "ferry_prefs"
    private const val PREF_KEY_DEVICE_UUID = "ferry_device_uuid"
}
