 ...
 *
 * WHY: MdnsService is the gateway between Ferry and macOS discovery.
 * If the mDNS registration is wrong (wrong service type, missing TXT records),
 * macOS will never show Ferry in the AirPlay menu. These tests verify that
...
class MdnsServiceTest {
     ...
     * device stays in the menu even when Ferry is not running.
     */
    @Test
     ...
     * macOS won't be able to connect to Ferry.
     */
    @Test
    ...
    fun `stop emits DISABLED protocol state`() {
        val states = mutableListOf<com.ferry.receiver.service.ProtocolState>()
        val service = MdnsService(mockContext, onStateChange = { states.add(it) })

        service.stop()

        assertTrue(states.contains(com.ferry.receiver.service.ProtocolState.DISABLED))
    }
