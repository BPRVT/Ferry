...
class MirrorStallEscalationTest {
    ...
    fun `a few failed rebuilds do not end the session`() {
        for (attempt in 1..4) {
            assertFalse(
                ...
                MirrorStreamServer.shouldRecycleAfterStall(attempt, now, lastRecycleMs = 0L)
            )
        }
    }
    ...
    fun `sustained failed rebuilds end the session`() {
        assertTrue(MirrorStreamServer.shouldRecycleAfterStall(5, now, lastRecycleMs = 0L))
        assertTrue(MirrorStreamServer.shouldRecycleAfterStall(179, now, lastRecycleMs = 0L))
    }
    ...
    fun `a recent recycle blocks another one`() {
        assertFalse(
            ...
            MirrorStreamServer.shouldRecycleAfterStall(50, now, lastRecycleMs = now - 10_000)
        )
    }
    ...
    fun `after the rate limit a still-broken session may recycle again`() {
        assertTrue(
            MirrorStreamServer.shouldRecycleAfterStall(5, now, lastRecycleMs = now - 61_000)
        )
    }
    ...
    fun `no failed rebuilds never escalates`() {
        assertFalse(MirrorStreamServer.shouldRecycleAfterStall(0, now, lastRecycleMs = 0L))
    }
}
