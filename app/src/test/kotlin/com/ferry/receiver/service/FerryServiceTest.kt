...
/**
 * FerryServiceTest — Unit tests for [FerryService].
 *
 * WHY: [FerryService] is the central coordinator. Bugs in its state aggregation
 ...
 *
 * HOW: [FerryService] is an Android [Service] and requires Android APIs for
...
class FerryServiceTest {
    ...
    fun `ACTION_START has package-qualified value`() {
        assertEquals("com.ferry.receiver.action.START", FerryService.ACTION_START)
    }

    @Test
    fun `ACTION_STOP has package-qualified value`() {
        assertEquals("com.ferry.receiver.action.STOP", FerryService.ACTION_STOP)
    }

    @Test
    fun `ACTION_RESTART has package-qualified value`() {
        assertEquals("com.ferry.receiver.action.RESTART", FerryService.ACTION_RESTART)
    }
    ...
    fun `NOTIFICATION_ID is positive`() {
        assertTrue(
            ...
            FerryService.NOTIFICATION_ID > 0
        )
    }

    @Test
    fun `CHANNEL_ID is non-empty`() {
        assertTrue(FerryService.CHANNEL_ID.isNotEmpty())
    }

    @Test
    fun `all three ACTION constants are distinct`() {
        val actions = setOf(
            FerryService.ACTION_START,
            FerryService.ACTION_STOP,
            FerryService.ACTION_RESTART
        )
    ...
    /** The exact mapping lambda from [FerryService.startAirPlay] — tested as a pure function. */
    ...
    // wiring between FerryService and MainActivity without needing Android.
