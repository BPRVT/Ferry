...
if [[ -z "$PACKAGE" ]]; then
  if adb shell pm path com.ferry.receiver.googletv >/dev/null 2>&1; then
    PACKAGE="com.ferry.receiver.googletv"
  elif adb shell pm path com.ferry.receiver.firetv >/dev/null 2>&1; then
    PACKAGE="com.ferry.receiver.firetv"
  else
    PACKAGE="com.ferry.receiver.googletv"
  fi
fi
